...
public class CustomFile extends StorageNode{
    ...
    public void helper_cp(CustomFolder parent){
        if(this.command_cp(parent.actualPath)){
            CustomFile copiedFile = this;
            copiedFile.updatePath(parent.path + "/" + this.name);
            parent.child_files.put(copiedFile.name, copiedFile);
            return;
        }
    }
    public native boolean command_cp(String path);
    public void helper_mv(CustomFolder currentParent,CustomFolder newParent){
        if(this.command_mv(newParent.actualPath)){
            CustomFile movedFile = this;
            movedFile.updatePath(newParent.path + "/" + movedFile.name);
            currentParent.child_files.remove(this.name);
            newParent.child_files.put(movedFile.name, movedFile);
            return;
        }
    }
    public  boolean command_mv(String path){
        if(this.command_cp(path)){
            this.command_rm();
            return true;
        }
        return false;
